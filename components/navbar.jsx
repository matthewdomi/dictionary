    ...
    <div className='shadow-md w-screen fixed top-0 left-0 '>
     <div className='md:flex items-center justify-between bg-white py-2 px-7 mr-4'>
        ...
        <div className='text-3xl absolute right-8 top-6 cursor-pointer md:hidden'>
            <GiHamburgerMenu onClick={()=>setOpen(!open)} className=" pb-1 hover:bg-slate-100 hover:rounded-2xl hover:p-1 hover:duration-700 hover:text-blue-500"/>
        ...
        <ul className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static bg-white md:z-auto z-[-1] left-0 w-full md:w-auto
         md:pl-0 pl-9 transition-all duration-700 ease-in ${open ? 'top-20 opacity-100' : "top-[-490px]" } md:opacity-100 ` }>
            {
            Links.map((Link)=>(
                <li key={Link.name} className='md:ml-8 text-md md:my-0 my-7 '>
                    <a href={Link.link} className='text-gray-800 hover:text-blue-500 decoration-sky-900'>{Link.name}</a>
                </li>
            ))}
             <div className='md:ml-8 text-md md:my-0 my-7  '><BsPersonCircle className='text-2xl text-blue-500 hover:bg-slate-100 cursor-pointer hover:rounded-3xl hover:p-1 hover:duration-700 hover:text-blue-700'/></div> 
